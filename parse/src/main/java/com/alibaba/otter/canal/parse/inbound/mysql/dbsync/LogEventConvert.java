...
public class LogEventConvert extends AbstractCanalLifeCycle implements BinlogParser<LogEvent> {
	...
	/**
	 * 数据库的列字段是否开启过滤，比如有些银行卡、手机号的信息需要替换成星号
	 */
	...
	private Entry parseQueryEvent(QueryLogEvent event) {
		...
		if (StringUtils.endsWithIgnoreCase(queryString, BEGIN)) {
			...
			if (result.getType() == EventType.ALTER || result.getType() == EventType.ERASE || result.getType() == EventType.CREATE
					|| result.getType() == EventType.TRUNCATE || result.getType() == EventType.RENAME || result.getType() == EventType.CINDEX
				...
				if (StringUtils.isEmpty(tableName) || (result.getType() == EventType.RENAME && StringUtils.isEmpty(result.getOriTableName()))) {
					...
					throw new CanalParseException("SimpleDdlParser process query failed. pls submit issue with this queryString: " + queryString
	...
	private boolean parseOneRow(RowData.Builder rowDataBuilder, RowsLogEvent event, RowsLogBuffer buffer, BitSet cols, boolean isAfter, TableMeta tableMeta)
		...
		if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
			...
			if (!existRDSNoPrimaryKey) {
				...
				if (tableMeta == null) {
					tableError = true;
					if (!filterTableError) {
						throw new CanalParseException("not found [" + event.getTable().getDbName() + "." + event.getTable().getTableName()
								+ "] in db , pls check!");
					}
				}
				...
				if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
					tableError = true;
					if (!filterTableError) {
						throw new CanalParseException("column size is not match for table:" + tableMeta.getFullName() + "," + columnInfo.length + " vs "
								+ tableMeta.getFileds().size());
					}
				}
			} else {
		...
		for (int i = 0; i < columnCnt; i++) {
			...
			if (buffer.isNull()) {
				...
				case Types.LONGVARBINARY:
					...
					if (fieldMeta != null && isText(fieldMeta.getColumnType())) {
						columnBuilder.setValue(textDataWash(new String((byte[]) value, charset)));
						javaType = Types.CLOB;
					} else {
			...
			// 设置是否update的标记位
			columnBuilder
					.setUpdated(isAfter && isUpdate(rowDataBuilder.getBeforeColumnsList(), columnBuilder.getIsNull() ? null : columnBuilder.getValue(), i));
			if (isAfter) {
	 ...
	 * @return str
	 */
	private String textDataWash(String src) {
		if (null == src) {
			return "";
		}
		src = src.trim();
	...
	private Header createHeader(String binlogFile, LogHeader logHeader, String schemaName, String tableName, EventType eventType) {
		...
		com.alibaba.otter.canal.protocol.CanalEntry.Pair.Builder pairHost = com.alibaba.otter.canal.protocol.CanalEntry.Pair.newBuilder();
		pairHost.setKey("dbAddress");
		pairHost.setValue(dbAddress.getHostName());

		com.alibaba.otter.canal.protocol.CanalEntry.Pair.Builder pairPort = com.alibaba.otter.canal.protocol.CanalEntry.Pair.newBuilder();
		pairPort.setKey("dbPort");
		pairPort.setValue(String.valueOf(dbAddress.getPort()));

		com.alibaba.otter.canal.protocol.CanalEntry.Pair.Builder pairIp = com.alibaba.otter.canal.protocol.CanalEntry.Pair.newBuilder();
		pairIp.setKey("dbIp");
		pairIp.setValue(dbAddress.getAddress().getHostAddress());

		headerBuilder.addProps(pairHost.build());
		headerBuilder.addProps(pairPort.build());
		headerBuilder.addProps(pairIp.build());
	...
	private boolean isText(String columnType) {
		return "LONGTEXT".equalsIgnoreCase(columnType) || "MEDIUMTEXT".equalsIgnoreCase(columnType) || "TEXT".equalsIgnoreCase(columnType)
