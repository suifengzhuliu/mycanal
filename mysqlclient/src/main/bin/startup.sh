...
then 
	...
	if [ $client_mode == "Cluster" ] ; then 
		...
		$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $CANAL_OPTS -classpath .:$CLASSPATH canal.alibaba.otter.canal.CanalClientMain 1>>$base/bin/nohup.out 2>&1 &
	fi
